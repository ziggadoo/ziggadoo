...
export type SearchRow = Priced & {
  ...
  is_sponsored: boolean; last_verified_at: string | null;
};

const ioLabel = { indoor: "Indoor", outdoor: "Outdoor", mixed: "In & out" } as const;

export default function VenueCard({ v, kidAges, adults, tagline }: { v: SearchRow; kidAges: number[]; adults: number; tagline?: string | null }) {
  ...
  return (
    <Link href={`/v/${v.slug}`} className="block rounded-3xl bg-white p-4 shadow-sm ring-1 ring-ink/10 transition hover:-translate-y-0.5 hover:shadow-md">
