...
export default function VenueCard({ v, kidAges, tagline, categories, saved, signedIn, back }: { v: SearchRow; kidAges: number[]; tagline?: string | null; categories?: string[] | null; saved?: boolean; signedIn?: boolean; back?: string }) {
  ...
  const heart = signedIn ? (
    <form action={toggleSaved} className="absolute right-3 top-3 z-10">
      <input type="hidden" name="venue_id" value={v.id} /><input type="hidden" name="slug" value={v.slug} /><input type="hidden" name="kind" value="saved" /><input type="hidden" name="on" value={saved ? "0" : "1"} /><input type="hidden" name="back" value={back ?? "/"} />
      <button aria-label={saved ? "Remove from saved" : "Save this place"} className={`flex h-8 w-8 items-center justify-center rounded-full text-base shadow-sm ring-1 ${saved ? "bg-sun ring-sun" : "bg-white ring-ink/10 text-ink/50"}`}>{saved ? "♥" : "♡"}</button>
    </form>
  ) : null;
  return (
    ...
    <Link href={`/v/${v.slug}`} className="flex min-w-0 gap-3 rounded-3xl bg-white p-3 shadow-sm ring-1 ring-ink/10 transition hover:-translate-y-0.5 hover:shadow-md">
      ...
      <div className="flex min-w-0 flex-1 flex-col">
        <div className={`flex items-start justify-between gap-2 ${signedIn ? "pr-8" : ""}`}>
