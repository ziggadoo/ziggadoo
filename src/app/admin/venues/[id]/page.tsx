...
import PhotoManager, { type PhotoRow } from "@/components/PhotoManager";
...
export default async function EditVenue({ params, searchParams }: { params: Promise<{ id: string }>; searchParams: Promise<{ msg?: string }> }) {
  ...
  const [{ data: reviews }, { data: photos }, { data: tickets }, { data: ll }] = await Promise.all([
    ...
    supabase.from("venue_photos").select("id, storage_path, caption, status, is_community, sort_order").eq("venue_id", id).order("sort_order"),
  ...
  return (
    <main className="mx-auto max-w-2xl px-4 pb-16 pt-6 sm:px-6">
      ...
      <form action={saveVenue} className="mt-4 grid gap-3 sm:grid-cols-2">
        ...
        <fieldset className="grid gap-1 rounded-2xl bg-white p-3 ring-1 ring-ink/10 sm:col-span-2"><legend className="px-1 text-xs font-bold uppercase tracking-wide text-ink/50">Good for</legend>{GOOD_FOR.filter((g) => g.key !== "free").map((g) => <C key={g.key} name={"gf_" + g.key} label={g.label} checked={cats.includes(g.key)} />)}<p className="text-xs text-ink/50">&quot;Free &amp; nearly free&quot; is automatic: free entry or a child price of 10 AED or less.</p></fieldset>
        ...
        <PhotoManager venueId={v.id} slug={v.slug} venueName={v.name} hero={v.hero_image_url} photos={(photos ?? []) as PhotoRow[]} />
      ...
      <h2 className="mt-6 text-sm font-bold uppercase tracking-wide text-ink/50">Parent photo uploads to check ({photos?.filter((p) => p.is_community).length ?? 0})</h2>
      <p className="mt-1 text-xs text-ink/60">Venue and admin photos are managed in the form above.</p>
      <ul className="mt-2 grid gap-2">
        {photos?.filter((p) => p.is_community).map((p) => (
