...
type Params = { ages?: string; indoor?: string; from?: string; near?: string; hs?: string; good?: string; sort?: string };
...
export default async function Home({ searchParams }: { searchParams: Promise<Params> }) {
  ...
  return (
    <main className="mx-auto max-w-2xl px-4 pb-16 pt-6 sm:px-6">
      ...
      <SearchBar ages={sp.ages ?? ""} indoor={sp.indoor ?? ""} from={start.key} near={nearMatch ? sp.near : ""} good={good?.key ?? ""} sort={sort} />
      ...
      <div className="mt-5 flex items-center justify-between gap-3">
        <p className="min-w-0 text-sm text-ink/60">
          {rows.length} places from {start.label}{kidAges.length ? `, for ${kidAges.map((m) => m < 12 ? "under 1" : m >= 168 ? "13+" : String(Math.round(m / 12))).join(", ")}` : ""}.
        </p>
      </div>
      ...
      <div className="mt-3 grid min-w-0 gap-3">
        {rows.map((v) => <VenueCard key={v.id} v={v} kidAges={kidAges} tagline={v.tagline} categories={v.categories} />)}
      </div>
